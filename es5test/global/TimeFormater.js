...
define(function(){
	...
	var TimeFormater = function () {
		function TimeFormater() {
			_classCallCheck(this, TimeFormater);
		}

		_createClass(TimeFormater, null, [{
			key: "getLocalTime",
	   ...
	   */
			value: function getLocalTime() {
				var millisecond = arguments.length <= 0 || arguments[0] === undefined ? 0 : arguments[0];
				var formatStr = arguments.length <= 1 || arguments[1] === undefined ? "#y/#m/#d" : arguments[1];
				var fillZero = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];

				if (millisecond < 0) {
					if (GlobalStaticConfing.isDebug) throw new Error("参数错误!");else return "";
				}
				...
				/*month  */
				if ($iMonth >= 0) {
					if (fillZero) $sMonth = $liveMonth + 1 < 10 ? '0' : '';
					$sMonth += String($liveMonth + 1);
					lowerStr = lowerStr.replace('#m', $sMonth);
				}
				/*day  */
				if ($iDay >= 0) {
					if (fillZero) $sDay = $liveDay < 10 ? '0' : '';
					$sDay += String($liveDay);
					lowerStr = lowerStr.replace('#d', $sDay);
				}
				/*  */
				if ($iHour >= 0) {
					if (fillZero) $sHour = $liveHour < 10 ? '0' : '';
				...
				/*  */
				if ($iMinute >= 0) {
					if (fillZero) $sMinute = $liveMinute < 10 ? '0' : '';
				...
				/*  */
				if ($iSecond >= 0) {
					if (fillZero) $sSecond = $liveSecond < 10 ? '0' : '';
	   ...
	   */
			...
			key: "countTimeFormatter",
			value: function countTimeFormatter() {
				var millisecond = arguments.length <= 0 || arguments[0] === undefined ? 0 : arguments[0];
				var formatStr = arguments.length <= 1 || arguments[1] === undefined ? "#h:#i:#s" : arguments[1];
				var keepZero = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];
				var fillZero = arguments.length <= 3 || arguments[3] === undefined ? true : arguments[3];
				...
				if ($iDay >= 0) $liveData -= $liveDay * $NUM_DAY;
				var $liveHour = $liveData / $NUM_HOUR;
				if ($iHour >= 0) $liveData -= $liveHour * $NUM_HOUR;
				var $liveMinute = $liveData / $NUM_MINUTE;
				if ($iMinute >= 0) $liveData -= $liveMinute * $NUM_MINUTE;
				...
				if ($iDay >= 0) {
					...
					if ($sDay != "00" && $sDay != "0" || keepZero) lowerStr = lowerStr.replace('#d', $sDay);else {
				...
				if ($iHour >= 0) {
					...
					if ($sHour != "00" && $sHour != "0" || keepZero || $sDay != "") lowerStr = lowerStr.replace('#h', $sHour);else {
				...
				if ($iMinute >= 0) {
					...
					if ($sMinute != "00" && $sMinute != "0" || keepZero || $sHour != "") lowerStr = lowerStr.replace('#i', $sMinute);else lowerStr = lowerStr.substr(lowerStr.indexOf("#s"));
				}
				if ($iSecond >= 0) {
					if (fillZero) $sSecond = $liveSecond < 10 ? '0' : '';
					if ($iMinute < 0 && $liveSecond <= 0 && $liveMilliSecond > 0) {
